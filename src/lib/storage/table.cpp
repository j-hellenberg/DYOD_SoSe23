...
Table::Table(const std::shared_ptr<const Table>& old_table_for_metadata, std::vector<std::shared_ptr<Chunk>>& chunks)
    : Table() {
...
void Table::_copy_metadata_from(const std::shared_ptr<const Table>& other_table) {
